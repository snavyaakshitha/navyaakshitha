...
export default function HomePage(): JSX.Element {
	...
	const birthday = new Date('2001-02-06');
	const isBirthday =
		today.getDate() === birthday.getDate() && today.getMonth() === birthday.getMonth();

	const description = `Transforming ideas into digital realities, as a Software Engineer.`;
			...
			<div className="min-h-screen flex items-center justify-center py-12">
				<div className="max-w-md sm:max-w-lg md:sm:max-w-2xl lg:sm:max-w-3xl w-full space-y-8 text-center">
					<Animate
						...
						I&apos;m <span className="block">Navya Akshitha,</span>{' '}<span className="hidden sm:block" />a{' '}
						...
						{ACTIONS.map((action, index) => {
							...
							return (
								<Animate
									...
									<Button.Outline href={action.href} target={action.external ? "_blank" : null}>
										{action.icon}
